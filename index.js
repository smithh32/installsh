...
// === FUNGSI BARU: Cek Genshin Impact (Ihsan API) ===
/**
 * Mengambil username Genshin Impact dari API Ihsan.
 * @param {string} id
 * @returns {object} { success, username, message }
 */
async function getGiUsername(id) {
    try {
        // Metode GET: parameter langsung dimasukkan ke URL
        const apiUrl = `https://api.isan.eu.org/nickname/gi?id=${id}`; 
        
        // Kirim sebagai GET request
        ...
        console.error('getGiUsername API error:', error.message);
        // Tangkap error spesifik dari API
        if (error.response && error.response.data && error.response.data.message) {
             return { success: false, message: `❌ Error API: ${error.response.data.message}` };
        }
        if (error.code === 'ECONNABORTED') {
             return { success: false, message: `❌ Gagal.\nPermintaan timeout (10 detik).` };
        }
        return { success: false, message: `❌ Gagal.\nTerjadi kesalahan: ${error.message}` };
    }
}

// === FUNGSI BARU: Instagram Downloader ===
/**
 * Mengambil link download media Instagram dari API Nekolabs.
...
async function getIgMedia(url) {
    try {
        ...
        // API Nekolabs (sesuai /subdofinder, tampaknya tidak perlu API key)
        const apiUrl = `https://api.nekolabs.web.id/downloader/instagram?url=${encodedUrl}`;
        
        const response = await axios.get(apiUrl, { 
            timeout: 60000 // 1 menit timeout (Downloader bisa lama)
        });
        ...
        if (data.success && data.result && data.result.downloadUrl) {
            ...
            return { success: false, message: data.message || "❌ Gagal men-download." };
        }

    } catch (error) {
        console.error('getIgMedia API error:', error.message);
        if (error.response && error.response.data && error.response.data.message) {
             return { success: false, message: `❌ Error API: ${error.response.data.message}` };
        }
        if (error.code === 'ECONNABORTED') {
...
// === FUNGSI BARU: Cek Magic Chess (Ihsan API) ===
/**
 * Mengambil username Magic Chess dari API Ihsan.
 * @param {string} id
 * @param {string} server
 * @returns {object} { success, username, message }
 */
async function getMcggUsername(id, server) {
    try {
        const apiUrl = 'https://api.isan.eu.org/nickname/mcgg'; // <-- URL Endpoint
        
        const postData = {
            "id": id,
            "server": server // <-- Butuh ID & Server
        };
        ...
        console.error('getMcggUsername API error:', error.message);
        if (error.response && error.response.data && error.response.data.message) {
             return { success: false, message: `❌ Error API: ${error.response.data.message}` };
        }
        if (error.code === 'ECONNABORTED') {
...
// === FUNGSI UPDATE: Cek Call of Duty (Ihsan API via GET) ===
/**
 * Mengambil username Call of Duty Mobile dari API Ihsan.
 * (Diubah ke metode GET karena POST gagal)
 * @param {string} id
 * @returns {object} { success, username, message }
 */
async function getCodmUsername(id) {
    try {
        // Kita ubah ke metode GET: parameter langsung dimasukkan ke URL
        const apiUrl = `https://api.isan.eu.org/nickname/cod?id=${id}`; 
        
        // Kirim sebagai GET request (bukan POST)
        const response = await axios.get(apiUrl, {
            timeout: 10000 // 10 detik timeout
        });
        ...
        console.error('getCodmUsername API error:', error.message);
...
// === FUNGSI UPDATE: Cek FF (Ihsan API) ===
/**
 * Mengambil username Free Fire dari API Ihsan.
 * @param {string} id
 * @returns {object} { success, username, message }
 */
async function getFfUsername(id) {
    try {
        const apiUrl = 'https://api.isan.eu.org/nickname/ff'; // <-- URL BARU
        
        // Data yang akan dikirim via POST
        const postData = {
            "id": id // <-- Hanya perlu ID
        };
        ...
        // Cek status sukses (mengikuti format Ihsan)
        if (data.success && data.name) {
            return { success: true, username: data.name }; // <-- Output diubah ke data.name
        } else {
        ...
        console.error('getFfUsername API error:', error.message);
        // Tangkap error spesifik dari API
        if (error.response && error.response.data && error.response.data.message) {
             return { success: false, message: `❌ Error API: ${error.response.data.message}` };
        }
...
// === FUNGSI UPDATE: Cek ML (Ihsan API) ===
/**
 * Mengambil username Mobile Legends dari API Ihsan.
 * @param {string} id
 * @param {string} zoneid
 * @returns {object} { success, username, message }
 */
async function getMlUsername(id, zoneid) {
    try {
        const apiUrl = 'https://api.isan.eu.org/nickname/ml'; // <-- URL BARU
        
        // Data yang akan dikirim via POST
        const postData = {
            "id": id,
            "server": zoneid // <-- Parameter diubah dari zoneid -> server
        };
        ...
        console.error('getMlUsername API error:', error.message);
        // Tangkap error spesifik dari API
        if (error.response && error.response.data && error.response.data.message) {
             return { success: false, message: `❌ Error API: ${error.response.data.message}` };
        }
...
// === FUNGSI UPDATE (FIXED): Search Image (Anabot API) ===
/**
 * Mencari gambar dari Google via API Anabot.
 * (Struktur JSON diperbaiki berdasarkan tes 'kadal')
...
async function getGoogleImage(query) {
    try {
        const encodedQuery = encodeURIComponent(query);
        const apiUrl = `https://anabot.my.id/api/search/gimage?query=${encodedQuery}&apikey=freeApikey`;
        
        const response = await axios.get(apiUrl, { timeout: 20000 });
        
        const data = response.data;
        
        // --- LOGIKA DIPERBAIKI (Sesuai JSON Anda) ---
        // Kita harus mengecek satu level lebih dalam ke 'data.data.result'
        if (data.success && data.data && data.data.result && data.data.result.length > 0) {
            
            // Array-nya adalah 'data.data.result'
            const images = data.data.result; 
            
            // Ambil 1 OBJEK gambar secara acak
            const randomImageObject = images[Math.floor(Math.random() * images.length)];
            
            // 'randomImageObject' adalah { url: "...", ... }
            // Kita kembalikan 'url'-nya
            return { success: true, imageUrl: randomImageObject.url };
        } else {
            // Ini yang terjadi sebelumnya
...
// === FUNGSI BARU: Image to Text (OCR) ===
/**
 * Mengambil teks dari gambar menggunakan API Anabot OCR.
 * @param {string} imageUrl URL gambar yang akan diproses
 * @returns {object} { success, result, message }
 */
async function getOcrResult(imageUrl) {
    try {
        // URL-encode diperlukan agar URL gambar aman dikirim sebagai parameter
        const encodedUrl = encodeURIComponent(imageUrl);
        
        // API ini menggunakan 'freeApikey' sesuai contoh Anda
        const apiUrl = `https://anabot.my.id/api/tools/ocr?imageUrl=${encodedUrl}&apikey=freeApikey`;
        
        const response = await axios.get(apiUrl, { 
            timeout: 30000 // Timeout 30 detik (OCR bisa butuh waktu lama)
        });
        ...
        if (data.success && data.data && data.data.result) {
            ...
            // Handle error dari API (misal: "player not found")
            return { success: false, message: data.message || "❌ Gagal memproses OCR." };
        }

    } catch (error) {
        console.error('getOcrResult API error:', error.message);
        // Tangkap error spesifik dari API
        if (error.response && error.response.data && error.response.data.message) {
             return { success: false, message: `❌ Error API: ${error.response.data.message}` };
        }
        if (error.code === 'ECONNABORTED') {
             return { success: false, message: `❌ Gagal.\nPermintaan OCR timeout (30 detik).` };
        }
        return { success: false, message: `❌ Gagal.\nTerjadi kesalahan: ${error.message}` };
    }
}
...
// === FUNGSI UPDATE: Subdomain Finder (FIXED) ===
/**
 * Mencari subdomain dari API Nekolabs.
 * (Diperbaiki: Dihapus 'apikey' yang tidak perlu)
 * @param {string} domain Domain yang akan dicari
...
async function getSubdomains(domain) {
    try {
        ...
        // APIKEY DIHAPUS - Sesuai cURL Anda
        const apiUrl = `https://api.nekolabs.web.id/tools/subdomain-finder?domain=${encodedDomain}`;
        ...
        if (data.success && data.result && Array.isArray(data.result)) {
            ...
            return { success: false, message: data.message || "❌ Gagal mencari subdomain." };
        }

    } catch (error) {
        console.error('getSubdomains API error:', error.message);
        if (error.response && error.response.data && error.response.data.message) {
             return { success: false, message: `❌ Error API: ${error.response.data.message}` };
        }
        if (error.code === 'ECONNABORTED') {
             return { success: false, message: `❌ Gagal.\nPermintaan timeout (120 detik).` };
        }
        return { success: false, message: `❌ Gagal.\nTerjadi kesalahan: ${error.message}` };
    }
}
...
// === FUNGSI UPDATE: AIV2 (OpenAI O4-Mini) ===
/**
 * Mengambil respons AI dari API Nekolabs (OpenAI O4-Mini).
 * @param {string} query Teks pertanyaan dari user
 * @param {string} sessionId ID unik untuk user (kita pakai userId)
 * @returns {object} { success, result, message }
 */
async function getAiV2Response(query, sessionId) { // <-- Nama fungsi diubah ke v2
    try {
        const encodedQuery = encodeURIComponent(query);
        
        // (URL ini 'o3' sesuai dengan yang ada di kode Anda)
        const apiUrl = `https://api.nekolabs.web.id/ai/openai/o3?text=${encodedQuery}&sessionId=${sessionId}`;
        ...
        if (data.success && data.result) {
            ...
            return { success: false, message: data.message || "❌ AI (v2) gagal merespons." }; // <-- Teks diubah ke v2
        }

    } catch (error) {
        console.error('getAiV2Response API error:', error.message); // <-- Log diubah ke v2
        
        if (error.response && error.response.status === 404) {
            return { success: false, message: "❌ Layanan AI (v2) tidak ditemukan (404)." }; // <-- Teks diubah ke v2
        }
        if (error.code === 'ECONNABORTED') {
             return { success: false, message: `❌ AI (v2) gagal merespons.\nPermintaan timeout (60 detik).` }; // <-- Teks diubah ke v2
        }
        return { success: false, message: `❌ Gagal menghubungi AI (v2).\nTerjadi kesalahan: ${error.message}` }; // <-- Teks diubah ke v2
    }
}
...
// === FUNGSI: AI (Gemini Flash 2.5) ===
/**
 * Mengambil respons AI dari API Nekolabs (Gemini Flash 2.5).
 * @param {string} query Teks pertanyaan dari user
 * @param {string} sessionId ID unik untuk user (kita pakai userId)
 * @returns {object} { success, result, message }
 */
async function getAiResponse(query, sessionId) {
    try {
        ...
        const apiUrl = `https://api.nekolabs.web.id/ai/gemini/2.5-flash/v2?text=${encodedQuery}&sessionId=${sessionId}`;
...
// === HANDLER: /searchimg (Image Search) - [FIXED PROXY] ===
bot.onText(/\/searchimg (.+)/, async (msg, match) => {
    ...
    try {
        ...
        // 3. Panggil fungsi API (ini tidak berubah)
        const result = await getGoogleImage(query);

        // 4. Handle Gagal (jika API Anabot tidak menemukan)
        if (!result.success) {
            return await bot.editMessageText(result.message, { // Cth: "Gambar tidak ditemukan"
                chat_id: chatId,
                message_id: loadingMsg.message_id,
                parse_mode: 'Markdown'
            });
        ...
        // 5. Handle Sukses - [INI LOGIKA BARUNYA]
        
        // Bot Anda akan mengunduh gambar dari URL yang didapat
        const imageUrl = result.imageUrl;
        
        // Kirim pesan loading kedua (download bisa butuh waktu)
        await bot.editMessageText(`✅ *Gambar ditemukan!* Mengunduh dari server...`, {
            chat_id: chatId, message_id: loadingMsg.message_id, parse_mode: 'Markdown'
        });

        // Unduh gambar sebagai 'arraybuffer'
        const imageResponse = await axios.get(imageUrl, {
            responseType: 'arraybuffer', // Ini penting
            timeout: 15000 // Timeout 15 detik untuk download
        });
        
        const imageBuffer = imageResponse.data; // Ini adalah data gambarnya

        // Kirim FOTO sebagai BUFFER (data)
        await bot.sendPhoto(chatId, imageBuffer, {
            caption: `✅ Gambar ditemukan untuk: \`${query}\``,
            parse_mode: 'Markdown'
        });
        ...
        // 'catch' ini sekarang akan menangkap error JIKA:
        // 1. API Anabot gagal.
        // 2. Download gambar (axios.get) gagal (cth: timeout atau link 404).
        console.error("Handler /searchimg error:", error.message);
        
        let errorText = '❌ Terjadi kesalahan internal saat mencari gambar.';
        
        // Beri pesan error yang lebih spesifik
        if (error.code === 'ECONNABORTED') {
            errorText = '❌ Gagal: Timeout saat mengunduh gambar dari server sumber.';
        } else if (error.response && error.response.status) {
            errorText = `❌ Gagal mengunduh gambar.\nServer sumber merespons: ${error.response.status}`;
        }
...
// === HANDLER: /subdofinder (Domain) ===
bot.onText(/\/subdofinder (\S+)/, async (msg, match) => {
    const chatId = msg.chat.id;
    const userId = msg.from.id;
    
    // Ambil domain (cth: google.com)
    ...
    try {
        // 2. Kirim pesan loading
        loadingMsg = await bot.sendMessage(chatId, `⏳ *Mencari subdomain untuk:* \`${domain}\`...\n\n_(Ini mungkin butuh 1-2 menit)_`, { parse_mode: 'Markdown' });
        ...
        // 5. Handle Sukses (LOGIKA TEKS PANJANG)
        
        // Gunakan 'Set' untuk menyimpan subdomain unik
        const allSubdomains = new Set();
        
        // Loop hasil API
        // 'entry.name_value' berisi string seperti "sub1.domain.com\nsub2.domain.com"
        result.results.forEach(entry => {
            const names = entry.name_value.split('\n'); // Pisah berdasarkan baris baru
            names.forEach(name => allSubdomains.add(name.trim())); // Tambahkan ke Set
        });

        // Hapus domain utama dari daftar jika ada
        allSubdomains.delete(domain);

        if (allSubdomains.size === 0) {
            return await bot.editMessageText(`❌ *Tidak ada subdomain yang ditemukan untuk \`${domain}\`.*`, {
                chat_id: chatId, message_id: loadingMsg.message_id, parse_mode: 'Markdown'
            });
        }

        // Ubah Set kembali ke array, lalu gabung jadi satu string
        const uniqueList = Array.from(allSubdomains);
        const resultText = uniqueList.join('\n');
        
        // Batas karakter aman Telegram adalah 4096
        const TELEGRAM_MAX_LENGTH = 4096;

        if (resultText.length > TELEGRAM_MAX_LENGTH) {
            // JIKA TERLALU PANJANG: Kirim sebagai file
            
            // Buat buffer dari string
            const fileBuffer = Buffer.from(resultText, 'utf-8');
            
            await bot.sendDocument(chatId, fileBuffer, {
                // Caption untuk dokumen
                caption: `✅ *Ditemukan ${uniqueList.length} subdomain unik untuk \`${domain}\`.*\n\n(Hasil terlalu panjang, dikirim sebagai file .txt).`,
                parse_mode: 'Markdown'
            }, {
                // Metadata file
            ...
            // JIKA AMAN (PENDEK): Kirim sebagai teks biasa
            
            await bot.editMessageText(
                `✅ *Ditemukan ${uniqueList.length} subdomain unik untuk \`${domain}\`*\n\n\`\`\`\n${resultText}\n\`\`\``, 
                {
                    chat_id: chatId,
                    message_id: loadingMsg.message_id,
                    parse_mode: 'Markdown'
        ...
        console.error("Handler /subdofinder error:", error.message);
        await bot.sendMessage(chatId, '❌ Terjadi kesalahan internal saat memproses /subdofinder.');
        if (loadingMsg) {
...
// === HANDLER: /aiv2 (OpenAI) ===
bot.onText(/\/aiv2 (.+)/, async (msg, match) => { // <-- Regex diubah ke /aiv2
    const chatId = msg.chat.id;
    const userId = msg.from.id;
    const query = match[1];
    ...
    try {
        // 2. Kirim pesan loading
        loadingMsg = await bot.sendMessage(chatId, `🧠 *AI (v2) sedang memikirkan jawaban...*\n\n_Harap tunggu..._`, { parse_mode: 'Markdown' }); // <-- Teks diubah ke v2

        // 3. Panggil fungsi API
        const result = await getAiV2Response(query, sessionId); // <-- Panggilan diubah ke v2
        ...
        console.error("Handler /aiv2 error:", error.message); // <-- Log diubah ke v2
        
        await bot.sendMessage(chatId, '❌ Terjadi kesalahan internal saat memproses /aiv2.'); // <-- Pesan diubah ke v2
...
// === HANDLER: /cekml (ID & Zone) ===
bot.onText(/\/cekml (\d+) (\d+)/, async (msg, match) => {
    const chatId = msg.chat.id;
    const userId = msg.from.id;
    ...
    try {
        // 2. Kirim pesan loading
        loadingMsg = await bot.sendMessage(chatId, `⏳ *Mencari username ML: ${id} (${zoneid})...*`, { parse_mode: 'Markdown' });

        // 3. Panggil fungsi API
        const result = await getMlUsername(id, zoneid);
        ...
        await bot.editMessageText(`✅ *Player Ditemukan!*\n\n*Username:* \`${username}\`\n*ID:* \`${id} (${zoneid})\``, {
            chat_id: chatId,
            message_id: loadingMsg.message_id,
            parse_mode: 'Markdown'
        });
        ...
        console.error("Handler /cekml error:", error.message);
        
        await bot.sendMessage(chatId, '❌ Terjadi kesalahan internal saat memproses /cekml.');
...
// === HANDLER: /mcgg (ID & Server) ===
bot.onText(/\/mcgg (\d+) (\d+)/, async (msg, match) => {
    const chatId = msg.chat.id;
    const userId = msg.from.id;
    
    const id = match[1];
    const server = match[2]; // <-- parameter kedua adalah server
    ...
    try {
        // 2. Kirim pesan loading
        loadingMsg = await bot.sendMessage(chatId, `⏳ *Mencari username Magic Chess: ${id} (${server})...*`, { parse_mode: 'Markdown' });

        // 3. Panggil fungsi API
        const result = await getMcggUsername(id, server);
        ...
        console.error("Handler /mcgg error:", error.message);
        await bot.sendMessage(chatId, '❌ Terjadi kesalahan internal saat memproses /mcgg.');
        if (loadingMsg) {
            try { await bot.deleteMessage(chatId, loadingMsg.message_id); } catch(e) {}
        }
    }
...
// === HANDLER: /gi (ID) ===
bot.onText(/\/gi (\d+)/, async (msg, match) => {
    const chatId = msg.chat.id;
    const userId = msg.from.id;
    
    const id = match[1]; // Hanya ada 1 grup (ID)
    ...
    try {
        // 2. Kirim pesan loading
        loadingMsg = await bot.sendMessage(chatId, `⏳ *Mencari username Genshin: ${id}...*`, { parse_mode: 'Markdown' });

        // 3. Panggil fungsi API
        const result = await getGiUsername(id); // Panggil fungsi GI
        ...
        console.error("Handler /gi error:", error.message);
        
        await bot.sendMessage(chatId, '❌ Terjadi kesalahan internal saat memproses /gi.');
...
// === HANDLER: /codm (ID) ===
bot.onText(/\/codm (\S+)/, async (msg, match) => { // <-- Pakai (\S+) untuk menangkap ID (Open ID bisa jadi panjang)
    const chatId = msg.chat.id;
    const userId = msg.from.id;
    ...
    try {
        // 2. Kirim pesan loading
        loadingMsg = await bot.sendMessage(chatId, `⏳ *Mencari username CODM: ${id}...*`, { parse_mode: 'Markdown' });

        // 3. Panggil fungsi API
        const result = await getCodmUsername(id); // Panggil fungsi CODM
        ...
        console.error("Handler /codm error:", error.message);
        await bot.sendMessage(chatId, '❌ Terjadi kesalahan internal saat memproses /codm.');
        if (loadingMsg) {
            try { await bot.deleteMessage(chatId, loadingMsg.message_id); } catch(e) {}
        }
    }
...
// === HANDLER: /cekff (ID) ===
bot.onText(/\/cekff (\d+)/, async (msg, match) => {
    const chatId = msg.chat.id;
    const userId = msg.from.id;
    ...
    try {
        // 2. Kirim pesan loading
        loadingMsg = await bot.sendMessage(chatId, `⏳ *Mencari username FF: ${id}...*`, { parse_mode: 'Markdown' });

        // 3. Panggil fungsi API
        const result = await getFfUsername(id); // Panggil fungsi FF
        ...
        console.error("Handler /cekff error:", error.message);
        
        await bot.sendMessage(chatId, '❌ Terjadi kesalahan internal saat memproses /cekff.');
...
// === HANDLER: /imgtotext (Reply to Image) ===
bot.onText(/\/imgtotext/, async (msg) => {
    const chatId = msg.chat.id;
    const userId = msg.from.id;
    ...
    // 2. Cek apakah pesan ini me-reply sesuatu
    if (!msg.reply_to_message) {
        return bot.sendMessage(chatId, '❌ Perintah ini harus digunakan dengan me-reply sebuah *gambar*.', { parse_mode: 'Markdown' });
    }

    // 3. Cek apakah yang di-reply adalah FOTO
    if (!msg.reply_to_message.photo) {
        return bot.sendMessage(chatId, '❌ Harap reply ke sebuah *gambar*, bukan stiker atau file lain.', { parse_mode: 'Markdown' });
    }

    let loadingMsg;
    try {
        // 4. Kirim pesan loading
        loadingMsg = await bot.sendMessage(chatId, `⏳ *Membaca teks dari gambar...*`, { parse_mode: 'Markdown' });

        // 5. Dapatkan link gambar dari Telegram
        const photo = msg.reply_to_message.photo;
        // Ambil file_id dari foto resolusi tertinggi
        const fileId = photo[photo.length - 1].file_id;
        // Minta URL download temporer ke Telegram
        const fileLink = await bot.getFileLink(fileId); 

        // 6. Panggil fungsi API dengan URL gambar
        const result = await getOcrResult(fileLink);
        ...
        // Kita gunakan format 'code' (```) agar rapi
        // karena hasil OCR seringkali berantakan
        await bot.editMessageText(
            `✅ *Teks Ditemukan:*\n\n\`\`\`\n${ocrText}\n\`\`\``, 
            {
                chat_id: chatId,
        ...
        console.error("Handler /imgtotext error:", error.message);
        
        await bot.sendMessage(chatId, '❌ Terjadi kesalahan internal saat memproses /imgtotext.');
...
// === HANDLER: /igdw (Instagram Downloader) ===
bot.onText(/\/igdw (\S+)/, async (msg, match) => {
    const chatId = msg.chat.id;
    const userId = msg.from.id;
    
    // Ambil URL (cth: https://www.instagram.com/...)
    const url = match[1]; 
    ...
    try {
        // 2. Kirim pesan loading
        loadingMsg = await bot.sendMessage(chatId, `⏳ *Mencari media Instagram...*\n\n${url}`, { parse_mode: 'Markdown' });

        // 3. Panggil fungsi API
        const response = await getIgMedia(url);
        ...
        const mediaUrls = result.downloadUrl;
        const isVideo = result.metadata.isVideo;
        const mediaType = isVideo ? 'video' : 'photo';
        
        // Buat caption (jika ada)
        let caption = result.metadata.caption ? `_${result.metadata.caption}_` : `✅ Downloaded by @${msg.from.username || 'user'}`;
        if (caption.length > 1024) caption = caption.substring(0, 1021) + '...'; // Batas caption Telegram

        await bot.editMessageText(`✅ *Media ditemukan!* (${mediaUrls.length} file)\n\nMengunduh ke server...`, {
            chat_id: chatId, message_id: loadingMsg.message_id, parse_mode: 'Markdown'
        });
        ...
        if (mediaUrls.length === 1) {
            ...
            // Download file ke buffer (Sama seperti /searchimg)
            const fileResponse = await axios.get(mediaUrl, { responseType: 'arraybuffer', timeout: 60000 });
            const fileBuffer = fileResponse.data;

            // Kirim berdasarkan tipe
            if (isVideo) {
                await bot.sendVideo(chatId, fileBuffer, { caption: caption, parse_mode: 'Markdown' });
            } else {
                await bot.sendPhoto(chatId, fileBuffer, { caption: caption, parse_mode: 'Markdown' });
            }
            
        } else {
            // --- LOGIKA ALBUM (MEDIA GROUP) ---
            
            // Download semua file (ini berat, tapi satu-satunya cara)
            const downloadPromises = mediaUrls.map(url => 
            ...
            // Buat array 'InputMedia' dari buffer
            const mediaGroup = downloadedFiles.map((fileResponse, index) => {
                const item = {
                    type: mediaType,
                    media: fileResponse.data // Kirim sebagai buffer
                };
                
                // Tambahkan caption HANYA di item pertama
                if (index === 0) {
        ...
        console.error("Handler /igdw error:", error.message);
        
        let errorText = '❌ Terjadi kesalahan internal saat memproses /igdw.';
        if (error.code === 'ECONNABORTED' || /timeout/i.test(error.message)) {
            errorText = '❌ Gagal: Timeout saat mengunduh file media.';
        } else if (/failed to get HTTP URL content/i.test(error.message)) {
             errorText = '❌ Gagal: Link media diblokir oleh server sumber. (Ini masalah umum)';
        }
...
// === HANDLER: /brat (GIF Maker) ===
bot.onText(/\/brat (.+)/, async (msg, match) => {
    const chatId = msg.chat.id;
    const userId = msg.from.id;
    
    // Ambil teks setelah perintah /brat
    const text = match[1]; 
    ...
    try {
        // 2. Kirim pesan loading
        loadingMsg = await bot.sendMessage(chatId, `⏳ *Membuat GIF 'brat' dengan teks:* "${text}"...`, { parse_mode: 'Markdown' });

        // 3. Siapkan URL (Tidak perlu 'axios', kita hanya bangun link)
        const encodedText = encodeURIComponent(text);
        const gifUrl = `https://anabot.my.id/api/maker/brat?text=${encodedText}&apikey=freeApikey`;

        // 4. Kirim GIF/Animasi menggunakan URL tersebut
        // Telegram akan otomatis mengambil file dari link
        await bot.sendAnimation(chatId, gifUrl, {
            caption: `✅ GIF "brat" berhasil dibuat!`
        });
        ...
        // Error ini bisa terjadi jika API Anabot down atau teksnya tidak valid
        console.error("Handler /brat error:", error.message);
        
        // Jika gagal, kita edit pesan loading menjadi pesan error
        await bot.editMessageText('❌ Terjadi kesalahan. Gagal membuat GIF.\nAPI mungkin sedang down atau teks tidak valid.', {
            chat_id: chatId,
            message_id: loadingMsg.message_id,
            parse_mode: 'Markdown'
        });
...
// === HANDLER: /qc (Image Quote Creator) ===
bot.onText(/\/qc (.+)/, async (msg, match) => {
    const chatId = msg.chat.id;
    const userId = msg.from.id;
    
    // Ambil teks setelah perintah /qc
    const text = match[1]; 
    ...
    try {
        // 2. Kirim pesan loading
        loadingMsg = await bot.sendMessage(chatId, `⏳ *Membuat gambar quote...*`, { parse_mode: 'Markdown' });

        // 3. Siapkan URL (Kita hardcode semua parameter KECUALI teks)
        const encodedText = encodeURIComponent(text);
        
        // URL ini berisi semua parameter dari cURL Anda
        const imageUrl = `https://anabot.my.id/api/maker/iqc?text=${encodedText}&chatTime=11%3A02&statusBarTime=17%3A01&bubbleColor=%23272a2f&menuColor=%23272a2f&textColor=%23FFFFFF&fontName=Arial&signalName=Telkomsel&apikey=freeApikey`;

        // 4. Kirim FOTO (bukan animasi) menggunakan URL tersebut
        // Telegram akan otomatis mengambil file dari link
        ...
        // Jika gagal, kita edit pesan loading menjadi pesan error
        await bot.editMessageText('❌ Terjadi kesalahan. Gagal membuat gambar quote.\nAPI mungkin sedang down atau teks tidak valid.', {
            chat_id: chatId,
            message_id: loadingMsg.message_id,
            parse_mode: 'Markdown'
        });
...
bot.onText(/\/ai (.+)/, async (msg, match) => {
    ...
    let loadingMsg;
    try {
        // 2. Kirim pesan loading (Ini aman, biarkan 'Markdown')
        loadingMsg = await bot.sendMessage(chatId, `🧠 *Sabar Monyet Lagi Mikir:* "${query}"...\n\n_Harap tunggu, ini mungkin butuh 10-30 detik._`, { parse_mode: 'Markdown' });

        // 3. Panggil fungsi API
        const result = await getAiResponse(query, String(userId));

        // 4. Handle Gagal (Ini aman, biarkan 'Markdown')
        if (!result.success) {
            return await bot.editMessageText(result.message, {
                chat_id: chatId,
                message_id: loadingMsg.message_id,
        ...
        // 5. Handle Sukses [INI YANG DIPERBAIKI]
        // 'parse_mode' dihapus agar Telegram tidak error
        // saat Gemini mengirim format Markdown yang rusak.
        await bot.editMessageText(result.result, {
            chat_id: chatId,
            message_id: loadingMsg.message_id
            // parse_mode: 'Markdown' <-- DIHAPUS
        });

    } catch (error) {
        // 'catch' ini akan menangkap error 'can't parse entities' jika terjadi lagi
        console.error("Handler /ai error:", error.message);
        
        if (error.response && error.response.data && error.response.data.description) {
             await bot.sendMessage(chatId, `❌ Gagal menampilkan respons AI:\n${error.response.data.description}`);
        } else {
